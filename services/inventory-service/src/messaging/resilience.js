...
function getMaxAttempts() {
  return positiveInteger(
    process.env.RABBITMQ_MAX_ATTEMPTS,
...
function parseEvent(message) {
  ...
  if (!event || typeof event !== "object") {
    throw nonRetryableError("Message body must be a JSON object");
  }

  if (!event.eventId || typeof event.eventId !== "string") {
    throw nonRetryableError("Event is missing a valid eventId");
  }

  if (!event.eventType || typeof event.eventType !== "string") {
    throw nonRetryableError("Event is missing a valid eventType");
  }

  if (!event.data || typeof event.data !== "object") {
    throw nonRetryableError("Event is missing data");
  }
...
async function retryOrDeadLetter(channel, message, queueConfig, error) {
  ...
  if (!error.nonRetryable && attempt < getMaxAttempts()) {
    ...
    console.warn(
      `Retry scheduled for ${queueConfig.queueName}; next delivery is attempt ${attempt + 1}`
    );
  } else {
    ...
    console.error(
      `Message moved to ${queueConfig.dlqQueue} after ${attempt} attempt(s): ${error.message}`
    );
  }
