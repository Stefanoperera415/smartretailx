...
async function deleteInventory(req, res) {
  try {
    const { productId, warehouseId } = req.params;
    const deleted = await inventoryRepo.deleteInventory(productId, warehouseId);
    if (!deleted) {
      return res.status(404).json({ error: "Inventory not found" });
    }
...
async function reserveInventory(req, res) {
  try {
    ...
    if (!updated) {
      return res
        .status(409)
        .json({ error: "Insufficient available inventory" });
    }
    return res
      .status(200)
      .json({ message: "Inventory reserved successfully", data: updated });
  } catch (error) {
...
async function releaseInventory(req, res) {
  try {
    ...
    if (!updated) {
      return res
        .status(409)
        .json({ error: "Unable to release requested quantity" });
    }
    return res
      .status(200)
      .json({ message: "Inventory released successfully", data: updated });
  } catch (error) {
