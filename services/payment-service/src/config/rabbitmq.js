...
function publishEvent(eventType, data) {
  ...
  getChannel().publish(EXCHANGE_NAME, eventType.toLowerCase(), Buffer.from(JSON.stringify(event)), {
    persistent: true,
...
module.exports = { connectRabbitMQ, getChannel, publishEvent, EXCHANGE_NAME };
